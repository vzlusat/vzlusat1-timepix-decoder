...
from_time = "24.05.2020 00:00:00"
to_time = "27.05.2020 00:00:00"
...
while i <= t_end:
    ...
    we_are_in = pxl_count > 1.5 # data-based
...
def plot_everything(*args):
    ...
    cb = m.colorbar(location="bottom", label="Ocekavan pixelova aktivita [$log_{10}$ px/s]") # draw colorbar
    ...
    plt.title('Plan mereni, {} - {}'.format(from_time, to_time))

    plt.subplots_adjust(left=0.025, bottom=0.05, right=0.975, top=0.95, wspace=0.1, hspace=0.1)

    plt.savefig(directory+"/{}_hybrid.png".format(from_time).replace(' ', '_').replace(':', '_'), dpi=200, bbox_inches='tight')
